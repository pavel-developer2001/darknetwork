import React from 'react'
import '../index.css'
import PostListItem from '../components/PostListItem'
import NetworkApi from '../apis/NetworkApi'
import {NetworkContext} from '../context/NetworkContext'

const AddPost = ({userId, userName}) => {
  const {addPosts} = React.useContext(NetworkContext)
  const [text, setText] = React.useState('')
  const [todos, setTodos] = React.useState([])





  const addForm = async (e) => {
    e.preventDefault();
 
    try{
      console.log(userId)
      const response = await NetworkApi.post(`/user/${userId}/addpost`, {postText: text, postAuthor:userName, userId})
      const postInfo = response.data.data
      const test = [...todos, text]
      setTodos(test)
      addPosts(postInfo)
      setText('')
    }catch(err){
      console.log(err)
    }
  }


    
    return (
      <>
        <div className='add-post'>
             <img className='user-icon' alt='icon' src="data:image/svg+xml;base64," />
             <form className='form-add-post'onSubmit={addForm} action='post'>
            <input 
              type="text"
              name='text'
              className="add-form"
              value={text}
              onChange={(e)=> setText(e.target.value)}
              placeholder="введите текст" />

                <button type='submit' className="comment-add-btn" onClick={addForm}>Добавить</button>
        </form>
        </div> 
        
        {todos.map((todo, index) => <PostListItem text ={todo} name={userName} like={0} commentCount={0} key={index} />)} 
        </>
    )
  }

export default AddPost  