  ...
  const addForm = async (e) => {
    ...
    try{
      ...
      const response = await NetworkApi.post(`/user/${userId}/addcomment`,
       {commentText: text, commentAuthor: userName, userId, postId})
        ...
        <div className='add-comment'>
            ...
            <input 
             ...
             placeholder="введите текст" />
                <button type='submit' className="comment-add-btn">Добавить</button>
        </form></div>
        {todos.map((todo, index) => <CommentListItem text={text} index={index} key={index} />)}
